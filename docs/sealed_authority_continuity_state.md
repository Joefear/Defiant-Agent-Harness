...
## Symmetric recoverability limit

Both stores already refused recovery files larger than 1 MiB. v0.60 applies the
same store-specific ceiling to canonical capture and `atomic_write_json()`.
Oversized proposed rotations fail before replacement, leaving the previously
valid generation intact. The durable JSON schemas remain unchanged and valid
existing state remains readable.

## Read-only projection

Command Core schema `0.58.0` publishes `authority_profile_state_bytes` and
