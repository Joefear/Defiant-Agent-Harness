...
v0.68 adds only the static launch-envelope-state byte ceiling and validated
snapshot posture. Existing launch cards remain sanitized to mode, environment
and working-directory hashes, counts, profile binding, and verification time.
The browser receives no environment names or values, secrets, paths, raw state,
unsafe-variable acknowledgements, profile rotation, process launch, repair, or
other mutation endpoint.

v0.69 adds only the static state-storage-observation byte ceiling and validated
snapshot posture. Existing state-storage cards remain sanitized to security
mode, root hash, permission, sync and ACL posture, profile binding, file counts,
and verification time. The browser receives no state-root path, raw ACL,
identity tuple, raw state, profile rotation, relocation, permission change,
repair, or other mutation endpoint.

v0.70 adds only independent static byte ceilings and validated snapshot
postures for control-plane isolation and workspace-root integrity. Existing
cards remain sanitized to hashes, modes, counts, relationship, profile binding,
...
v0.72 adds only the independently reconstructed completed-manifest verification
...
v0.74 adds only sanitized active-publication commitment posture and static
commitment-verification posture. The dashboard can say whether exact target
commitments are recorded or unavailable on a legacy crash intent, and can show
a commitment mismatch. It receives no individual commitment hashes, raw store
values, or prepare, replay, complete, repair, acceptance, or migration endpoint.

v0.75 adds only sanitized completed-checkpoint commitment posture and static
checkpoint-verification posture. During mixed-generation recovery the dashboard
can distinguish recorded from legacy-unavailable checkpoint commitments and can
show a checkpoint-store mismatch. It receives no individual hashes or raw store
values and gains no prepare, replay, completion, repair, acceptance, migration,
or other mutation endpoint.

v0.76 adds stable completed-checkpoint commitment verification and one static
...
v0.78 adds only sanitized predecessor and originating-intent transition-link
posture plus one static validation flag. The dashboard can distinguish
`verified`, `legacy_unavailable`, `not_applicable`, and `invalid`, but receives
