...
function renderResourceLimits(limits, configuration) {
  ...
  const details = [
    ...
    `state storage observation ${bytes(limits.state_storage_state_bytes)}`,
    `control-plane isolation state ${bytes(limits.control_plane_isolation_state_bytes)}`,
    `workspace integrity state ${bytes(limits.workspace_integrity_state_bytes)}`,
  ...
  if (configuration) {
    ...
    const authorityContinuity = configuration.sealed_authority_continuity_state
      ? "authority profile and operator trust state retained as sealed bounded snapshots with defensive projections"
    ...
    const authorityPublicationRecordSeals = configuration.validated_authority_publication_record_seals
      ? "authority publication intents and checkpoints validate sealed semantic record hashes"
      : "authority publication record seals unverified";
    const authorityPublicationTransitionLinks = configuration.validated_authority_publication_transition_links
      ? "authority publication transitions bind intents to prior checkpoints and checkpoints to originating intents"
      : "authority publication transition links unverified";
    ...
    const completedAuthorityPublicationCheckpointCommitments = configuration.verified_completed_authority_publication_checkpoint_store_commitments
      ? "stable completed authority publication checkpoints are checked against exact retained store commitments before reuse"
      : "completed authority publication checkpoint commitment verification unavailable";
    ...
    const stateStorageState = configuration.validated_state_storage_state_snapshot
      ? "state storage assurance validation and publication use one detached bounded state snapshot with symmetric I/O limits"
      : "state storage snapshot integrity unverified";
    const controlPlaneIsolationState = configuration.validated_control_plane_isolation_state_snapshot
      ? "control-plane isolation validation and publication use one detached bounded state snapshot with symmetric I/O limits"
      : "control-plane isolation state snapshot integrity unverified";
    const workspaceIntegrityState = configuration.validated_workspace_integrity_state_snapshot
      ? "workspace-root validation and publication use one detached bounded state snapshot with symmetric I/O limits"
      : "workspace integrity state snapshot unverified";
...
function renderStateIntegrity(integrity) {
  ...
  elements.stateIntegrityDetail.textContent = integrity.safe_to_execute
    ? publication && publication.state === "recovery_required"
      ? `Authority publication for generation ${integer.format(publication.generation)} requires exact replay (${label(publication.verification)}; intent seal ${label(publication.intent_record_seal)}; checkpoint seal ${label(publication.checkpoint_record_seal)}; prior-checkpoint link ${label(publication.intent_checkpoint_link)}; originating-intent link ${label(publication.checkpoint_intent_link)}; target commitments ${label(publication.store_commitments)}; checkpoint commitments ${label(publication.checkpoint_store_commitments)}). This dashboard remains read-only.`
...
function renderAuthorityProfile(
  ...
  const publicationDetail = publication
    ? `${label(publication.state)} · ${label(publication.verification)} · Intent seal ${label(publication.intent_record_seal)} · Checkpoint seal ${label(publication.checkpoint_record_seal)} · Prior-checkpoint link ${label(publication.intent_checkpoint_link)} · Originating-intent link ${label(publication.checkpoint_intent_link)} · Target commitments ${label(publication.store_commitments)} · Checkpoint commitments ${label(publication.checkpoint_store_commitments)} · ${shortId(publication.manifest_hash)}`
    : "Not recorded";
