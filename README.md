...
v0.89 streams history through one existing-log descriptor and keeps only the
requested bounded display rows, rather than retaining the whole decoded log.
Every record is still validated and no output is printed until the read
succeeds. Early failures close the stream. See `docs/streaming_history.md` for
memory, concurrency, and full-log processing limits.

v0.90 also streams show and verify. Show retains only its first matching record;
verify retains chain state and the first hash failure. Both still read the full
...
v0.92 enforces the existing export byte ceiling while accumulating pretty JSON,
instead of constructing the full serialized document before checking its size.
Exact output bytes, including ASCII escaping and the trailing newline, remain
unchanged. Nothing is published until encoding succeeds. Compact canonical
encoding and signing formats are unchanged. This does not cap selected-record
memory or individual encoder tokens. See `docs/incremental_export_encoding.md`.

v0.93 extends incremental output accumulation to compact exports, including
the pre-signing and pre-verification size checks. Existing canonical
normalization still runs in full, then identical canonical chunks are checked
